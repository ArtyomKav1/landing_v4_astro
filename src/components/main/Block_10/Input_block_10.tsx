...
export const InputBlock = () => {
    ...
    const postData = async (data: { name: string, company: string, number: string, email: string, check: boolean }) => {
        setErrorFetch(false)
        setLoading(true)
        try {
            ...
            <div className=" md:px-[48px] p-[20px] pb-[88px] max-w-[1224px] mx-auto">
                <div className="bg-[#8C26EA]/20 py-[16px] rounded-[24px]">
                    <div className="   max-sm:rounded-[20px] relative bg-[#8C26EA] px-[56px] max-sm:px-[20px] rounded-[24px] py-[64px] flex gap-[24px]">
                        {errorFetch && <div className="absolute top-[-60px] left-0 w-full flex items-center justify-center">
                            <div className=" bg-red-600 text-white py-[6px] px-[10px] rounded-[8px]">Ошибка отправки запроса.</div>
                        </div>}
                        {!popup
                            ? <div className="w-full flex max-lg:flex-col gap-[24px] ">
                                <div className="text-[#FFFFFF] lg:flex-1/2   w-full shrink-0  flex  flex-col gap-[12px]">
                                    <div className="text-[32px] leading-[40px] font-semibold tracking-[-1px] max-sm:text-[22px] max-sm:leading-[24px]  max-sm:tracking-[0px]">Попробуйте PravoTech Big Data в действии</div>
                                    <div className="text-[16px] opacity-80 leading-[24px]">Мы свяжемся с вами и обсудим, как интегрировать данные в вашу внутреннюю систему.</div>
                                </div>
                                <div className="w-full flex-1/2   flex flex-col gap-[12px] relative">
                                    {loading && <div className="absolute inset-0 bg-black/50 z-50 rounded-[12px] flex flex-col items-center justify-center gap-[20px]">
                                        <img src="/Block_10/Loader_white.svg" className="animate-spin " width={40} alt="" />
                            ...
                            : <div className="flex flex-col items-center justify-center text-center text-white/80 max-sm:h-[597px] gap-[16px] px-[36px] py-[24px]">
                                <img src="/Block_10/checkResponse.svg" className="" width={40} alt="" />
                                <div className="flex flex-col  gap-[8px]">
                                    <div className="text-white text-[22px] leading-[28px]">Заявка принята</div>
                                    <div>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</div>
                                </div>
                                <div className="underline" onClick={() => setPopup(false)}>Заполнить форму повторно</div>
                            </div>}
